#!/usr/bin/env python3
"""
Training script for CBAM-Bottleneck Only Model
CBAM blocks only in bottleneck, no attention in encoder or decoder
"""
...
def train_cbam_bottleneck_only():
    """Train CBAM-Bottleneck Only model"""
    
    print("🧠 Training CBAM-Bottleneck Only Model")
    print("=" * 50)
    
    # Configuration
    config = {
        'model_type': 'cbam_bottleneck_only',
        'in_channels': 4,
        'base_filters': 32,
        'num_classes': 3,
        'batch_size': 2,
        ...
        'output_dir': 'outputs/cbam_bottleneck_only'
    }
    ...
    # Create model
    print("\n🏗️ Creating CBAM-Bottleneck Only model...")
    model = UNet3D(
        in_channels=config['in_channels'],
        base_filters=config['base_filters'],
        num_classes=config['num_classes'],
        attention_type='cbam_bottleneck_only'  # Custom attention type
    ).to(config['device'])
